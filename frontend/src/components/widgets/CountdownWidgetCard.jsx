    ...
    <div className="flex w-full flex-col items-center">
      <div className="relative aspect-square w-full">
        ...
        <div className="absolute inset-0 flex items-center justify-center text-center">
          <span
            className={`font-black leading-none tracking-tight ${valueClassName}`}
...
export default function CountdownWidgetCard(props) {
  ...
  const countdown = useMemo(
    () => getCountdownProgressData(widget.data, now),
    [widget.data, now],
  );
  ...
  if (!countdown) {
    return (
      <WidgetCardShell
        ...
        controlsTone="light"
        className="@container p-[clamp(0.9rem,4.5cqw,1.45rem)]"
        overlay={
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(255,255,255,0.22),transparent_42%)] pointer-events-none" />
        }
      >
        <div className="mt-2 flex h-full flex-col">
          <h3 className="pr-16 font-outfit text-[clamp(1rem,5.2cqw,1.6rem)] font-black leading-[1.05] truncate whitespace-nowrap">
            Compte a rebours
          </h3>
  ...
  const isImageBackground =
    snapshot.appearance?.backgroundType === COUNTDOWN_APPEARANCE_TYPES.IMAGE &&
  ...
  const trackColor =
    textColor === "#1F2937" ? "rgba(31,41,55,0.15)" : "rgba(255,247,237,0.18)";
  const progressColor =
    textColor === "#1F2937" ? "rgba(31,41,55,0.96)" : "rgba(255,247,237,0.98)";
  const labelColor =
    textColor === "#1F2937" ? "rgba(31,41,55,0.72)" : "rgba(255,247,237,0.82)";
      ...
      {widget.size === "RECT" ? (
        <div className="grid h-full min-h-0 grid-rows-[auto_minmax(0,1fr)_auto] gap-[clamp(0.3rem,2.2%,0.85rem)] pt-[1%]">
          <div className="min-w-0 pr-[28%]">
            <h3 className="truncate whitespace-nowrap font-outfit text-[clamp(0.88rem,2.45cqw,1.45rem)] font-black leading-[1.02]">
              {snapshot.title}
            </h3>
          </div>
          ...
          <p className="max-w-[78%] truncate text-[clamp(0.4rem,0.82cqw,0.66rem)] leading-[1.1] font-semibold opacity-80">
            {recurrenceLabel}
          </p>
        </div>
      ) : (
        <div className="flex h-full min-h-0 flex-col">
          <div className="min-w-0 pr-[34%] pb-[1%]">
            <h3 className="truncate whitespace-nowrap font-outfit text-[clamp(0.78rem,3.8cqw,1.08rem)] font-black leading-none md:text-[clamp(0.82rem,2.8cqw,1.16rem)]">
              {snapshot.title}
            </h3>
          </div>
          ...
          <div className="flex flex-1 min-h-0 flex-col items-center justify-center gap-[4%] py-[1%]">
            <div className="w-[50%] max-w-[9.2rem] min-w-0">
              <ProgressRing
                ...
                labelColor={labelColor}
                strokeWidth={8}
                valueClassName="text-[clamp(1rem,6cqw,2.15rem)]"
              />
            </div>
          ...
          <p className="hidden min-[1280px]:block max-w-[78%] truncate text-[clamp(0.5rem,1.2cqw,0.68rem)] font-semibold opacity-80">
            {recurrenceLabel}
          </p>
        </div>
      )}
